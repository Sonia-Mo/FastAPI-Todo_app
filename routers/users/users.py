...
from routers.users.schemas import UserIn, UserOut, User
...
async def get_all_users(user: UserIn = Depends(get_current_user), db: Session = Depends(get_db), skip: int = 0, limit: int = 100):
...
async def create_user(new_user: UserIn, db: Session = Depends(get_db)):
    db_user = db.query(models.User).filter(models.User.username == new_user.username).first()
    ...
    db_user = models.User(username=new_user.username, email=new_user.email, hashed_password=hashed_password)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
